...
cmake ../ -DCMAKE_BUILD_TYPE=RELEASE -DWITH_CUDA=OFF -DBUILD_PYTHON_SUPPORT=ON -DPYTHON_EXECUTABLE=$anaconda_root/bin/python$version -DPYTHON_INCLUDE_DIR=$anaconda_include/python$version -DPYTHON_LIBRARY=$anaconda_lib/python$version.so -DPYTHON_NUMPY_INCLUDE_DIRS=$anaconda_lib/python$version/site-packages/numpy/core/include -DPYTHON_PACKAGES_PATH=$anaconda_lib/python$version/site-packages/numpy/core/include -DCMAKE_INSTALL_PREFIX=$install_prefix/ -DPNG_LIBRARY_RELEASE=$anaconda_lib/libpng.so -DJPEG_LIBRARY_RELEASE=$anaconda_lib/libjpeg.so -DTIFF_LIBRARY_RELEASE=$anaconda_lib/libtiff.so
